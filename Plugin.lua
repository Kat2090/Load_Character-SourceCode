...
TextBox.FocusLost:Connect(function(enterPressed)
	if enterPressed == true then
		...
		if Sucess then
			Remote:FireServer(Input, RigType)
			PlayerName.Text = game.Players:GetNameFromUserIdAsync(Input)
			PlayerID.Text = Input
			local content, isReady = game.Players:GetUserThumbnailAsync(Input, Enum.ThumbnailType.AvatarThumbnail, Enum.ThumbnailSize.Size100x100)
